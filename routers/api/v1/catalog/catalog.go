...
func getCatalogStatsOpts(ctx *context.APIContext) *door43metadata.SearchCatalogOptions {
	...
	if stageStr != "" {
		...
		if !ok {
			ctx.APIError(http.StatusUnprocessableEntity, fmt.Sprintf("invalid stage [%s]", stageStr))
			return nil
		}
	}
	...
	if startDateStr := ctx.FormString("startDate"); startDateStr != "" {
		...
		if err != nil {
			ctx.APIError(http.StatusUnprocessableEntity, fmt.Sprintf("invalid startDate: %v", err))
			return nil
		}
	...
	if endDateStr := ctx.FormString("endDate"); endDateStr != "" {
		...
		if err != nil {
			ctx.APIError(http.StatusUnprocessableEntity, fmt.Sprintf("invalid endDate: %v", err))
			return nil
		}
		if dateOnly {
...
// GetCatalogStatsExt get aggregate counts plus per-field entry counts of the catalog entries matching the given criteria
func GetCatalogStatsExt(ctx *context.APIContext) {
	// swagger:operation GET /catalog/stats-ext catalog catalogGetStatsExt
	// ---
	// summary: Aggregate counts plus entry counts per subject, flavor type, flavor, owner, language and metadata type of the catalog entries matching the given criteria. Same parameters as /catalog/stats
	// produces:
	// - application/json
	// parameters:
	// - name: q
		...
		if searchModeMap, ok := searchOrderByMap[sortOrder]; ok {
			for _, sortMode := range sortModes {
				if orderBy, ok := searchModeMap[strings.ToLower(sortMode)]; ok {
					...
					ctx.APIError(http.StatusUnprocessableEntity, fmt.Sprintf("invalid sort mode: \"%s\"", sortMode))
					return
				}
			}
		} else {
			ctx.APIError(http.StatusUnprocessableEntity, fmt.Sprintf("invalid sort order [%s]", sortOrder))
			return
		}
	} else {
