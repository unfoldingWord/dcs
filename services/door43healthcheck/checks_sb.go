...
func checkSBIngredients(ctx context.Context, dm *repo_model.Door43Metadata) []*repo_model.Door43HealthcheckIssue {
	...
	for _, path := range paths {
		...
		if size := entry.GetSize(ctx, gitRepo); size != ingredient.Size {
			mismatch(path, fmt.Sprintf("has a size of %d in metadata.json but the file is %d bytes", ingredient.Size, size))
		}
		...
		if md5sum := ingredient.Checksum["md5"]; md5sum != "" && dm.RefType == "tag" {
			actual, err := blobMD5(ctx, entry.Blob(gitRepo))
			if err != nil {
...
func blobMD5(ctx context.Context, blob *git.Blob) (string, error) {
