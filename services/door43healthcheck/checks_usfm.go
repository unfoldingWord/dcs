...
func checkUSFMBooks(ctx context.Context, dm *repo_model.Door43Metadata) []*repo_model.Door43HealthcheckIssue {
	...
	for _, ingredient := range dm.Ingredients {
		...
		issues = append(issues, checkUSFMBook(ctx, gitRepo, commit, dm, ingredient.Path, ingredient.Identifier)...)
	}
	return issues
}

// checkUSFMBook validates one USFM book file at the given path
func checkUSFMBook(ctx context.Context, gitRepo *git.Repository, commit *git.Commit, dm *repo_model.Door43Metadata, path, identifier string) []*repo_model.Door43HealthcheckIssue {
	blob, err := commit.GetBlobByPath(ctx, gitRepo, path)
	if err != nil || blob == nil {
