...
func checkRelationLanguages(_ context.Context, dm *repo_model.Door43Metadata) []*repo_model.Door43HealthcheckIssue {
	...
	for _, relation := range dm.Relations {
		if relation.Language != dm.Language && relation.Language != "hbo" && relation.Language != "el-x-koine" && dm.Repo.Owner.LowerName != "unfoldingword" {
			issues = append(issues, newIssue(repo_model.IssueCodeRelation, repo_model.SeverityLevelError,
...
// checkRelationCatalog verifies each relation resolves to a catalog entry and, for Bible/TSV
// subjects, that this resource's books exist in the relation's ingredients. It queries the
// door43_metadata table directly rather than calling the catalog API over HTTP.
func checkRelationCatalog(ctx context.Context, dm *repo_model.Door43Metadata) []*repo_model.Door43HealthcheckIssue {
	...
	for _, relation := range dm.Relations {
		...
		relDM := getRelationCatalogEntry(ctx, owner, fmt.Sprintf("%s_%s", relation.Language, relation.Identifier), relation.Version)
		if relDM == nil {
			issues = append(issues, newIssue(repo_model.IssueCodeRelationMissing, repo_model.SeverityLevelError,
				fmt.Sprintf("Relation %s does not exist in the DCS catalog", relation.FullRelation),
				fmt.Sprintf("Verify that the relation %s exists and is properly cataloged", relation.FullRelation)))
			continue
		}
