...
func (aReq *ArchiveRequest) Stream(ctx context.Context, repo *repo_model.Repository, w io.Writer) error {
	...
	if repoDM.MetadataType == "sb" {
		return gitrepo.CreateArchive(ctx, repo, aReq.Type.String(), w, setting.Repository.PrefixArchiveFiles, aReq.CommitID, nil)
	}
	...
	if err := cloneRepositoryAtCommit(ctx, repo.RepoPath(), aReq.RefShortName, aReq.CommitID, cloneDir); err != nil {
...
func doArchive(ctx context.Context, r *ArchiveRequest) (*repo_model.RepoArchiver, error) {
	...
	if archiver != nil {
		if archiver.Status == repo_model.ArchiverGenerating {
			return nil, nil //nolint:nilnil // return nil because the archive is still being generated; Await polls for it
		}
	} else {
