		...
		if !opts.TopicOnly {
			...
			for _, v := range strings.Split(opts.Keyword, ",") {
				...
				if opts.TeamID == 0 && strings.Count(opts.Keyword, "/") == 1 {
					...
					ownerName := pieces[0]
					repoName := pieces[1]
					likes = likes.Or(builder.And(builder.Like{"`repository`.owner_name", strings.ToLower(ownerName)}, builder.Like{"`repository`.lower_name", strings.ToLower(repoName)}))
				}
				if opts.IncludeDescription {
					likes = likes.Or(builder.Like{"LOWER(`repository`.description)", strings.ToLower(v)})
				}
				/*** DCS Customizations ***/
				likes = likes.Or(getMetadataCondByDBType(setting.Database.Type, v, opts.IncludeMetadata))
