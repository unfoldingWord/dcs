	...
	for _, lang := range languages {
		for _, v := range strings.Split(lang, ",") {
			if partialMatch {
				...
				langCond = langCond.Or(builder.Like{"CONCAT(SUBSTRING_INDEX(`repository`.lower_name, '_', 1), '_')", strings.ToLower(v) + "\\_"})
			} else {
				...
				langCond = langCond.Or(builder.Eq{"CONCAT(SUBSTRING_INDEX(`repository`.lower_name, '_', 1), '_')": strings.ToLower(v) + "_"})
			}
		}
	}
	return langCond
